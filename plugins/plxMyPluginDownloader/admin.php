...
# echo 'version = '.$version.' - remote version = '.$remote_version;
...
if($repo = plxMyPluginDownloader::getRepository($cache_dir.$repository_xmlfile)) {
	...
	foreach($repo as $plugName => $plug) {
		...
		$update=false;
		if(isset($plxAdmin->plxPlugins->aPlugins[$plugName]['instance']))
			$update = version_compare($plxAdmin->plxPlugins->aPlugins[$plugName]['instance']->getInfo('version'), $plug['version'], "<");
